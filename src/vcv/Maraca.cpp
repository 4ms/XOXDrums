...
#include "modules/info/Maraca_info.hh"
...
struct MaracaVcv : rack::Module {
	Maraca cores[PORT_MAX_CHANNELS];
	...
	MaracaVcv() {
		auto cnt = ElementCount::count<MaracaInfo>();
		config(cnt.num_params, cnt.num_inputs, cnt.num_outputs, cnt.num_lights);

		VCVModuleParamCreator<MaracaInfo> creator{this};
		for (auto &element : MaracaInfo::Elements)
...
struct MaracaWidget : rack::app::ModuleWidget {
	MaracaWidget(MaracaVcv *module) {
		setModule(module);
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, MaracaInfo::svg_filename.data())));
		...
		VCVWidgetCreator<MaracaInfo> creator(this, module);
		for (auto &element : MaracaInfo::Elements)
			std::visit([&creator](auto &el) { creator.create(el); }, element);
	}
};

rack::Model *modelMaraca = rack::createModel<MaracaVcv, MaracaWidget>("Maraca");
