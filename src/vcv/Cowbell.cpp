...
#include "modules/info/Cowbell_info.hh"
...
struct CowbellVcv : rack::Module {
	Cowbell cores[PORT_MAX_CHANNELS];
	...
	CowbellVcv() {
		auto cnt = ElementCount::count<CowbellInfo>();
		config(cnt.num_params, cnt.num_inputs, cnt.num_outputs, cnt.num_lights);

		VCVModuleParamCreator<CowbellInfo> creator{this};
		for (auto &element : CowbellInfo::Elements)
...
struct CowbellWidget : rack::app::ModuleWidget {
	CowbellWidget(CowbellVcv *module) {
		setModule(module);
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, CowbellInfo::svg_filename.data())));
		...
		VCVWidgetCreator<CowbellInfo> creator(this, module);
		for (auto &element : CowbellInfo::Elements)
			std::visit([&creator](auto &el) { creator.create(el); }, element);
	}
};

rack::Model *modelCowbell = rack::createModel<CowbellVcv, CowbellWidget>("Cowbell");
